...
from kite_app.config import API_SECRET, EXCHANGE, PRODUCT, round_to_tick
...
def buy(req: BuyOrderRequest) -> BuyOrderResponse:
    ...
    if req.order_type == "LIMIT":
        ...
        params["market_protection"] = (
            req.market_protection if req.market_protection is not None else -1
        )

    order_id = kite.place_order(**params)
    seed_order(order_id, req.symbol, req.qty)
...
def gtt_preview(order_id: str, target_pct: float, sl_pct: float) -> GttPreviewResponse:
    ...
    kite = get_kite()
    quote = kite.quote(f"{EXCHANGE}:{order['symbol']}")
    ltp = quote[f"{EXCHANGE}:{order['symbol']}"]["last_price"]
